...
def bayes(X, labels):
    ...
    stds = [np.std(mnozina, axis=0) for mnozina in mnoziny]

    # p(c) priorni ppsi trid
    pc = [len(mnozina)/m for mnozina in mnoziny]

    return means, stds, pc


def klasifikuj(posl, means, stds, pc):
    """
...
def pxc(bod, mean, std):
    ...
    ppst = 1
    for j in range(dims):
        exponent = math.exp(-((bod[j] - mean[j]) ** 2 / (2 * std[j] ** 2)))
        ppst_in_dim = (1 / (math.sqrt(2 * math.pi) * std[j] ** 2)) * exponent
        ppst *= ppst_in_dim
    return ppst
...
if __name__ == '__main__':
    ...
    grid_labels = klasifikuj(grid, means, stds, pc)
