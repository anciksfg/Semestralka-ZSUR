...
def train_multiple_fcns(Y, labels, num_epochs=20, beta=0.5):
    ...
    for i in range(pocet_trid):
        ...
        labels_i[labels == i] = 1
        labels_i[labels != i] = 0
        q, vyvoj_ceny = train(Y, labels_i, num_epochs=num_epochs, beta=beta)
        diskr_fce.append(q)
...
def train(X, labels, num_epochs, beta):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(m):
            ...
            grad = -bod * (label - predicted)/2
            # updatovat vahovou matici
            # kolikrat se vektor pricte
            b = abs(np.dot(q.T, bod) / beta)
            # b = np.floor(b)
            # print('b', b)
            c = beta * b / (np.dot(bod.T, bod))
            q = q - c * grad
...
if __name__ == '__main__':
    ...
    diskr_fce, vyvoje_cen = train_multiple_fcns(Y, labels, num_epochs=20, beta=0.05)
